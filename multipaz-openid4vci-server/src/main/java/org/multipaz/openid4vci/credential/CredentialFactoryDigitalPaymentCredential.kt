...
internal class CredentialFactoryDigitalPaymentCredential : CredentialFactory {
    ...
    override suspend fun mint(
        ...
        val credentialIdValue = if (paymentData.hasKey("credential_id")) {
            paymentData["credential_id"].asTstr
        } else {
            "cred-${credentialId.bucket.takeLast(6)}-${credentialId.index}"
        }
        ...
        val issuerNamespaces = buildIssuerNamespaces {
            addNamespace(DigitalPaymentCredential.CARD_NAMESPACE) {
                addDataElement("credential_id", Tstr(credentialIdValue))
                addDataElement("issuer_id", paymentData.getValueOrDefault("issuer_id", "issuer-bank-utopia"))
                ...
                addDataElement(
                    ...
                    paymentData.getValueOrDefault("payment_instrument_type", "card")
                )
                addDataElement("network", paymentData.getValueOrDefault("network", "visa"))
                addDataElement(
                ...
                addDataElement(
                    ...
                    paymentData.getValueOrDefault("wallet_binding_method", "device_key")
                )
                addDataElement(
                    "assurance_level",
                    paymentData.getValueOrDefault("assurance_level", "high")
                )
    ...
    override suspend fun display(systemOfRecordData: DataItem): CredentialDisplay {
        val baseUrl = BackendEnvironment.getBaseUrl()
        val givenName = if (systemOfRecordData["core"].hasKey("given_name")) {
            systemOfRecordData["core"]["given_name"].asTstr
        } else {
            "Cardholder"
        }
        return CredentialDisplay(
            title = "$givenName's Payment Card",
            cardArtUrl = "$baseUrl/card-generic.png"
        )
    }
