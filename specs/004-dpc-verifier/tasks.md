...
- [ ] T009 [P] Implement `DpcVerificationResult.kt` in `src/main/kotlin/org/multipaz/dpc/verifier/DpcVerificationResult.kt` — data class with fields: verified (Boolean), credentialFormat (String), instrumentId, holderName, maskedAccountRef, issuerName, txnDataVerified (Boolean), trustWarning (String?), error (String?), errorDescription (String?). Annotate with `@Serializable` using snake_case `@SerialName` annotations.
...
- [ ] T023 [US3] Write test `ResultHandlerTest.kt` in `src/test/kotlin/org/multipaz/dpc/verifier/ResultHandlerTest.kt` — test: verified session returns 200 with payment claims; pending session returns 202 with Retry-After; pending session with eudiCompat returns 400; expired session returns 404; declined session returns 200 with `user_declined` error. Test that `GET {path}/sdk.js` returns JavaScript content with correct Content-Type.
